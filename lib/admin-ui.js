...
nav.ops{margin:16px 0;padding:12px;background:#f8f9fa;border-radius:8px;display:flex;flex-wrap:wrap;gap:8px}
nav.ops a{padding:8px 14px;border-radius:6px;text-decoration:none;color:#2d6a4f;border:1px solid #2d6a4f;font-size:14px}
...
function renderNav(active, token) {
  ...
  const links = tabs
    .map(
      (tab) =>
        `<a href="${tab.href}" class="${active === tab.id ? "active" : ""}">${tab.label}</a>`
    )
    .join("");
  return `<nav class="ops">${links}</nav>`;
}

function renderPage({ title, active, token, body, flash = "" }) {
