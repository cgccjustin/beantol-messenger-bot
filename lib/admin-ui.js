...
.shop-switcher{margin:0 0 12px;padding:10px 12px;background:#f0f7f4;border:1px solid #c8e6d4;border-radius:8px}
`;
}
...
const ADMIN_TAB_PATHS = {
  overview: "/admin",
  handoffs: "/admin/handoffs/view",
  webhooks: "/admin/webhook-log",
  instagram: "/admin/instagram-setup/view",
  analytics: "/admin/analytics/view",
  sales: "/admin/sales/view",
  leads: "/admin/leads/view",
  appointments: "/admin/appointments/view",
  orders: "/admin/orders/view",
  quotes: "/admin/quotes/view",
  inventory: "/admin/inventory/view",
  closures: "/admin/closures/view",
  tools: "/admin/tools/view",
};

function renderBookmarkHint(token, req, tenantId = "") {
  const base = req ? `${req.protocol}://${req.get("host")}` : "";
  const tid = String(tenantId || req?.query?.tenant || "").trim();
  const path = adminUrl("/admin", token) + tenantQuery(tid);
  const full = base ? `${base}${path}` : path;
  return `<div class="bookmark-hint"><strong>Bookmark this admin hub:</strong> <a href="${escapeHtml(path)}">${escapeHtml(full)}</a>${tid ? ` · Shop: <strong>${escapeHtml(tid)}</strong>` : ""}</div>`;
}
...
function adminTenantsForSwitcher(tenants) {
  return (tenants || []).filter((t) => t?.enabled !== false && t?.google?.leadsSheetId);
}

/** Global shop picker — keeps you on the same tab when switching stores. */
function renderTenantSwitcher(token, activeTenantId, tenants, activeTab = "overview") {
  const list = adminTenantsForSwitcher(tenants);
  if (list.length <= 1) return "";
  const path = ADMIN_TAB_PATHS[activeTab] || "/admin";
  const base = `${path}?token=${encodeURIComponent(token || "")}`;
  const options = list
    .map(
      (t) =>
    ...
    .join("");
  return `<form class="filters shop-switcher"><label><strong>Shop:</strong> <select onchange="location.href='${base}&tenant='+encodeURIComponent(this.value)">${options}</select></label> <span class="muted">All tabs below show this store&apos;s CRM data.</span></form>`;
}
...
function renderPage({
  ...
  const bookmarkHtml = bookmark ? renderBookmarkHint(token, req, resolvedTenant) : "";
  const tenantList = tenants || [];
  const switcherHtml =
    tenantList.length > 1
      ? renderTenantSwitcher(token, resolvedTenant, tenantList, active)
      : "";
  return `<!DOCTYPE html>
