 ...
 */
/**
 * Returns a nudge for an ACTIVE (recently-touched) post-quote session.
 * Used when faith fires despite an active Beantol quote order session.
 */
function buildActivePostQuoteFaithNudge(senderId) {
  const session = getSession(senderId);
  if (!session || session.step === "closed") return "";
  const ref = session.lastCustomerAt || session.updatedAt;
  if (Date.now() - ref > POST_QUOTE_IDLE_MS) return ""; // idle → use buildFaithPostQuoteNudge
  const stepHints = {
    pickup_pending: `just reply OK to confirm pickup`,
    delivery_collect: `send your delivery address, name, and mobile`,
    closed: "",
  };
  const hint = stepHints[session.step] || `we can finalize your order`;
  return `\n\nP.S. Coming back to your order — your quote is still here. Whenever you're ready: ${hint}! 😊`;
}
