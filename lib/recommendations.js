...
function getInStockFallbackPicks(session) {
  const brew = session.brew || "unsure";
  const roast = brew === "filter" ? "filter" : "espresso";
  return CATALOG_PRODUCTS.filter((p) => p.roast === roast && !isProductIdOutOfStock(p.id))
    .slice(0, 2)
    .map((p) => ({
      id: p.id,
      why: "Available now — a solid match for what you described.",
    }));
}
...
function buildPicks(session) {
  ...
  let filtered = picks.filter((p) => {
    ...
    seen.add(p.id);
    return !isProductIdOutOfStock(p.id);
  });

  if (!filtered.length) {
    filtered = getInStockFallbackPicks(session);
  }
...
function buildRecommendationSystemNote() {
  return (
    ...
    "Only recommend beans IN STOCK per INVENTORY — never mention OUT OF STOCK beans."
  );
}
