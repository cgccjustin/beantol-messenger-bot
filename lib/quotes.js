...
const { buildQuoteFromText, formatPeso, parseLineItemsFromText } = require("./pricing");
const {
  getLeadsSheetId,
  getQuotesSheetTab,
...
function isQuoteCaptureConfigured() {
  return isQuoteCaptureEnabledForTenant() && hasGoogleCredentials();
}

function getQuotesTab() {
  return getQuotesSheetTab();
}
...
function shouldCreateQuote(payload) {
  ...
  if (stage === "browsing") return false;
  if (
...
function mergeQuoteLineItems(existingSummary, quoteData, extraSources, options = {}) {
  const byProduct = new Map();

  const existingText = String(existingSummary || "").replace(/\s·\s/g, "\n");
  if (existingText.trim()) {
    for (const line of parseLineItemsFromText(existingText, options)) {
      byProduct.set(line.productId, line);
    }
  }

  for (const line of quoteData.lineItems || []) {
    byProduct.set(line.productId, line);
  }
  ...
  if (!lines.length) return quoteData;
...
async function recordQuote(payload) {
  ...
  if (!quoteSnapshot) {
    ...
    mergedQuote = mergeQuoteLineItems(
      existingItems,
      mergedQuote,
      [userText, ...historyTexts, assistantReply],
      { wholesale }
    );
  }
  const quoteId = isNew ? generateQuoteId(rows) : existing?.[COL.QUOTE_ID] || generateQuoteId(rows);
  const shareToken = isNew ? makeShareToken() : existing?.[COL.SHARE_TOKEN] || makeShareToken();
