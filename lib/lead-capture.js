...
function normalizePhilippineMobile(raw) {
  ...
  if (digits.length === 12 && /^639\d{9}$/.test(digits)) return `0${digits.slice(2)}`;
  if (digits.length === 10 && /^9\d{9}$/.test(digits)) return `0${digits}`;
  // Fat-finger: 12 digits starting with 09 — use first 11 when valid
  if (digits.length === 12 && /^09\d{10}$/.test(digits)) {
...
function normalizePhoneInputText(text) {
  return String(text || "")
...
function extractPhone(text) {
  const t = normalizePhoneInputText(text);
  if (!t.trim()) return "";

  const myNumber = t.match(
    /\b(?:my|ang|akong|ako(?:ng)?)\s+(?:cell(?:phone)?|mobile|phone|cp|contact|num(?:ber)?)\s*(?:no\.?|number|num)?\s*(?:is|kay|ni)?\s*([+\d\s().-]{10,20})/i
  );
  if (myNumber) {
    const normalized = normalizePhilippineMobile(myNumber[1]);
    if (normalized) return normalized;
  }

  const labeled = t.match(
    /\b(?:mobile|cell(?:phone)?|phone|contact|number|cp|tel(?:ephone)?|no\.?)[#:\s-]*([+\d\s().-]{10,20})/i
  );
  if (labeled) {
    const normalized = normalizePhilippineMobile(labeled[1]);
    if (normalized) return normalized;
  }
  ...
  if (contiguous) {
    const normalized = normalizePhilippineMobile(contiguous[0]);
    if (normalized) return normalized;
  }

  const chunks = t.match(/(?:\+?\d[\d\s().-]{8,}\d)/g) || [];
  for (const chunk of chunks) {
    const normalized = normalizePhilippineMobile(chunk);
    if (normalized) return normalized;
  }
...
function detectFulfillment(text) {
  ...
  if (/\b(?:deliver(?:y|ed|ies|ing)?|maxim|padala|hatod|shipping|ship)\b/i.test(t)) {
    return "delivery";
  }
  return "";
}
