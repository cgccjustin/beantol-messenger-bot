...
async function ensureHeadersAndSeed(sheets, spreadsheetId, tab) {
  ...
  if (rows.length <= 1) {
    ...
    if (resolveProfile(tenant) !== "beantol") {
      console.log(
        `Inventory auto-seed skipped for ${tenant.id} (café tenant) — use /admin/inventory/reseed?tenant=${tenant.id}`
      );
      return;
    }
    if (refusesRoasterySeed(tenant)) {
      console.warn(`Inventory auto-seed blocked for ${tenant.id} — wrong catalog`);
      return;
    }
