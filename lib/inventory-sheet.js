...
function buildCacheFromItems(items) {
  const unavailableItems = items.filter((i) => i.status === "out_of_stock");
...
async function ensureInventoryLoaded(timeoutMs = 8000) {
  if (!isInventorySheetConfigured()) return;
  const cache = getTenantCache();
  if (cache.at && Date.now() - cache.at < CACHE_MS) return;

  const loadPromise = loadInventory(!cache.at);
  let timeoutId;
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(() => reject(new Error("inventory load timeout")), timeoutMs);
  });

  try {
    await Promise.race([loadPromise, timeoutPromise]);
  } catch (err) {
    console.warn("ensureInventoryLoaded:", err.message);
  } finally {
