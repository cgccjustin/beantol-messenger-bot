...
/** Only Beantol roastery may auto-append seed rows — never café/custom tenants. */
function shouldAutoSeedInventory(tenant, spreadsheetId) {
  const t = tenant || getActiveTenant();
  if (resolveProfile(t) !== "beantol" || t?.id !== "beantol") return false;
  const expectedSheet = getLeadsSheetId(t);
  if (expectedSheet && spreadsheetId !== expectedSheet) {
    console.warn(
      `Inventory auto-seed blocked: sheet ${spreadsheetId} is not ${t.id} CRM (${expectedSheet})`
    );
    return false;
  }
  return true;
  ...
  if (rows.length <= 1) {
    ...
    if (!shouldAutoSeedInventory(tenant, spreadsheetId)) {
      if (resolveProfile(tenant) !== "beantol") {
        console.log(
          `Inventory auto-seed skipped for ${tenant?.id || "tenant"} — use /admin/inventory/reseed?tenant=${tenant?.id || ""}`
        );
      }
      return;
    }
