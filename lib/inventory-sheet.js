...
/** Auto-seed empty Inventory tab from tenant catalog (Beantol roastery or café menu SKUs). */
function shouldAutoSeedInventory(tenant, spreadsheetId) {
  const t = tenant || getActiveTenant();
  const expectedSheet = getLeadsSheetId(t);
  if (expectedSheet && spreadsheetId !== expectedSheet) {
    console.warn(
      `Inventory auto-seed blocked: sheet ${spreadsheetId} is not ${t?.id || "tenant"} CRM (${expectedSheet})`
    );
    return false;
  }

  if (resolveProfile(t) === "beantol" && t?.id === "beantol") return true;

  const products = getCatalogProducts(t);
  if (products.length && !refusesRoasterySeed(t)) return true;
...
async function ensureHeadersAndSeed(sheets, spreadsheetId, tab) {
  ...
  if (rows.length <= 1) {
    ...
    if (!shouldAutoSeedInventory(tenant, spreadsheetId)) {
      if (!getCatalogProducts(tenant).length) {
        console.log(
          `Inventory auto-seed skipped for ${tenant?.id || "tenant"} — no catalog products defined`
        );
      }
      return;
    }
    ...
    if (!products.length || refusesRoasterySeed(tenant)) {
      console.warn(`Inventory auto-seed blocked for ${tenant?.id} — no valid catalog`);
      return;
    }
    ...
    console.log(`Inventory auto-seed [${tenantId}] → sheet ${spreadsheetId} (${products.length} products)`);
...
function resolveProductId(productIdCell, nameCell) {
  ...
  if (idRaw) {
    const byId = products.find((p) => p.id === idRaw);
    if (byId) return byId.id;
    const byKey = findCatalogProduct(idRaw, tenant);
    if (byKey) return byKey.id;
  }
  const nameRaw = String(nameCell || productIdCell || "").trim();
  if (nameRaw) {
    const byName = findCatalogProduct(nameRaw, tenant);
...
function rowToItem(row) {
  ...
  const catalog = getCatalogProductById(productId, getActiveTenant());
  return {
    productId,
...
function resolveCatalogLabel(item) {
  const product = getCatalogProductById(item.productId, getActiveTenant());
...
async function loadInventory(force = false) {
  ...
  return {
    ...
    rawRowCount,
    dataRowCount,
    wrongCatalog: inventoryHasWrongCatalogRows(items, getActiveTenant()),
  };
}
...
/** Replace Inventory tab rows with this tenant's menu catalog (fixes wrong Beantol seed). */
async function reseedInventoryFromCatalog() {
  if (!isInventorySheetConfigured()) {
  ...
  if (!products.length) {
    return {
      skipped: true,
      reason: `No inventory catalog for tenant ${tenant?.id || "unknown"}`,
    };
  }
  if (refusesRoasterySeed(tenant)) {
    return {
      skipped: true,
      reason: `Refusing to seed roastery SKUs for café tenant ${tenant.id}`,
    };
  }
  ...
  getTenantCache().at = 0;
  console.log(
    `Inventory reseeded [${tenant.id}]: ${products.map((p) => p.label).join(", ")}`
  );
  return {
    ok: true,
    tenantId: tenant.id,
