...
const { appendPostQuoteFulfillmentPrompt } = require("./post-quote-flow");
const { ADD_TO_ORDER_PATTERN } = require("./lead-capture");
const { gateQuoteProposal } = require("./inventory-availability");
...
function isQuoteConfirmYesTurn(userText, senderId, lastAssistantReply = "") {
  if (!isConfirmYes(userText)) return false;
  if (getSession(senderId)?.step === "confirm") return true;
  return assistantAlreadyAskedConfirm(lastAssistantReply);
}
...
function detectWholesale(userText, assistantReply = "", recentTexts = [], sessionQuote = null) {
  ...
  const customerOnly = [userText, ...recentTexts].filter(Boolean).join(" ");
  if (/\b(?:wholesale|bulk)\b/i.test(customerOnly)) return true;
  const kg = resolveOrderKg(customerOnly) || parseKgAmount(customerOnly);
  return kg != null && kg >= WHOLESALE_MOQ_KG;
}

function userTextHasExplicitSizeOrKg(userText, recentTexts = []) {
  const combined = [userText, ...recentTexts.slice(-2)].filter(Boolean).join("\n");
  if (/\b(?:250g|500g|1kg|\d+(?:\.\d+)?\s*kg)\b/i.test(combined)) return true;
  if (parseKgAmount(combined)) return true;
  const lines = parseLineItemsFromText(String(userText || ""), { quoteUserText: userText });
  return lines.some((line) => line.size && line.size !== "wholesale");
}
...
function buildGatedProposal(userText, assistantReply = "", options = {}) {
  return gateQuoteProposal(userText, assistantReply, buildProposal(userText, assistantReply, options));
}
...
function assistantAlreadyAskedConfirm(assistantReply) {
  ...
  if (/\bproceed with this order\b/i.test(t)) return true;
  if (/\bconfirm this order\b/i.test(t)) return true;
  return false;
}
...
function shouldOfferQuoteConfirm(userText, assistantReply, signal, options = {}) {
  ...
  if (effective?.kg >= WHOLESALE_MOQ_KG) {
    return Boolean(buildGatedProposal(userText, assistantReply, options));
  }
  ...
  const proposal = buildGatedProposal(userText, assistantReply, options);
  if (!proposal?.lineItems?.length) return false;

  if (explicit) return true;

  if (signal?.stage === "ordering" || signal?.stage === "wholesale") {
    return true;
  }

  if (
    /\b(?:order|buy|purchase|checkout|mag order|gusto ko order|place order|kuha ko)\b/i.test(
      userText
    )
  ) {
    return true;
  ...
  if (userTextHasExplicitSizeOrKg(userText, recentTexts)) {
    return true;
  }

  // Do not append quote confirm for availability or price-only turns without a chosen size/kg.
  return false;
}
...
async function finalizeQuoteFromYes(senderId, userText, platform, publicBaseUrl, quoteSnapshot, meta = {}) {
  ...
  const reply = appendPostQuoteFulfillmentPrompt(
    senderId,
    {
      platform,
      quoteSummary: quoteSnapshot?.summary || "",
      quoteSubtotal: quoteSnapshot?.subtotal ?? null,
      quoteUrl: url || "",
    },
    baseReply
  );
...
async function processQuoteConfirmPreAi(
  ...
  if (!session && isConfirmYes(userText) && assistantAlreadyAskedConfirm(lastAssistantReply)) {
    const proposal = buildGatedProposal(userText, lastAssistantReply, { recentTexts: [] });
  ...
  if (isConfirmYes(userText)) {
    const gatedQuote = gateQuoteProposal(userText, lastAssistantReply, session.quote);
    if (!gatedQuote?.lineItems?.length) {
      clearQuoteConfirmSession(senderId);
      return {
        handled: true,
        reply:
          "Sorry — that item is not available for order right now. Tell me which in-stock bean and size you would like instead.",
      };
    }
...
function processQuoteConfirmPostAi(senderId, userText, platform, assistantReply, context = {}) {
  ...
  if (effectiveKg?.kg >= WHOLESALE_MOQ_KG) {
    const wholesaleProposal = buildGatedProposal(userText, assistantReply, options);
  ...
  if (requestedBelowMoqBulkKg(userText, recentTexts, existingSession?.quote)) {
    const retailProposal = buildGatedProposal(userText, assistantReply, options);
  ...
  if (session?.step === "confirm") {
    if (isConfirmationOnly(userText)) {
      if (!isConfirmYes(userText)) {
        const refreshed = buildGatedProposal(userText, assistantReply, {
          ...options,
    ...
    if (kgMerged) {
      const previousQuote = session.quote;
      const gatedQuote = gateQuoteProposal(userText, assistantReply, kgMerged.quote);
      if (!gatedQuote?.lineItems?.length) return { handled: false };
      session.quote = gatedQuote;
      session.meta.wholesale = gatedQuote.lineItems.some((l) => l.size === "wholesale");
      session.updatedAt = Date.now();
      sessions.set(scopeKey(senderId), session);
      const upgrade = detectWholesaleUpgrade(previousQuote, gatedQuote);
      const prefix = upgrade ? formatWholesaleUpgradePrefix(upgrade) : "";
      return {
        handled: true,
        appendConfirm: true,
        reply: formatConfirmMessage(gatedQuote, prefix),
      };
    }

    if (isAddToQuote(userText)) {
      const addition = buildGatedProposal(userText, assistantReply, options);
      if (addition) {
        const merged = gateQuoteProposal(
          userText,
          assistantReply,
          mergeSessionCart(session.quote, addition, options)
        );
        if (!merged?.lineItems?.length) return { handled: false };
        session.quote = merged;
        session.updatedAt = Date.now();
        sessions.set(scopeKey(senderId), session);
        return {
          handled: true,
    ...
    const replacement = buildGatedProposal(userText, assistantReply, options);
