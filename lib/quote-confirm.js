...
function detectWholesale(userText, assistantReply = "", recentTexts = []) {
  if (requestedBelowMoqBulkKg(userText, recentTexts)) return false;
...
function buildProposal(userText, assistantReply = "", options = {}) {
  ...
  const quoteUserText = [userText, ...recentTexts.slice(-3)].filter(Boolean).join("\n");
  const opts = {
    ...options,
  ...
  const fromUser = buildQuoteFromText(userText, {
    ...opts,
    historyTexts: recentTexts.slice(-3),
    assistantReply: "",
  });
...
function shouldOfferQuoteConfirm(userText, assistantReply, signal, options = {}) {
  ...
  if (requestedBelowMoqBulkKg(userText, recentTexts)) {
...
function clearSessionIfBelowMoqRequest(senderId, userText, recentTexts = []) {
  const belowMoq = requestedBelowMoqBulkKg(userText, recentTexts);
  if (belowMoq != null) {
...
async function processQuoteConfirmPreAi(
  ...
  clearSessionIfBelowMoqRequest(senderId, userText, recentTexts);
