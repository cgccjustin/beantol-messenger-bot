...
const FAILED_CACHE_TTL_MS = Number(process.env.META_PROFILE_FAILED_CACHE_MINUTES || 5) * 60 * 1000;
...
async function resolveCustomerDisplayName(senderId, platform = "messenger") {
  ...
  try {
    ...
    if (!response.ok || data.error) {
      ...
      profileCache.set(cacheKey, { name: "", at: Date.now(), failed: true });
      return "";
    }

    const name = formatProfileName(data, platform);
    profileCache.set(cacheKey, { name, at: Date.now(), failed: !name });
    if (name) {
