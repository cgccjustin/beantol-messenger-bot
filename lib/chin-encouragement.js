...
const CAFE_OPERATIONAL_PATTERN =
...
/**
 * Offbeat "general" persona: faith mode only for non-café chat (life, faith, feelings).
 * Beantol board_exam persona: always faith mode (full channel).
 */
function shouldSkipFaithEncouragementForMessage(userText, tenant, recipient, options = {}) {
  if (!recipient || recipient.persona === "board_exam") return false;
  ...
  if (isCafeOrderFlowActive(senderId)) return true;
  ...
  if (CAFE_SIMPLE_GREETING.test(text)) return true;
  if (CAFE_OPERATIONAL_PATTERN.test(text)) return true;
