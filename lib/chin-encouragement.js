...
function buildChinEncouragementSystemPrompt(profileName) {
  ...
  return (
    ...
    `- Every reply must feel fresh. If she asks the same or similar thing again (bored again, fear again, "can I pass?" again), ` +
    `still answer fully with faith + a Bible verse — but use a NEW verse, a NEW angle, and NEW practical encouragement.\n` +
    `- Never repeat a verse reference you already used in this conversation. Never copy-paste or lightly reword a prior reply.\n` +
    `- You may briefly acknowledge she asked before ("Still feeling that way?") then offer something different — another book of the Bible, ` +
    `a new study tip, a prayer focus, or a different truth (peace vs strength vs joy vs wisdom).\n` +
    `- Rotate tone: sometimes gentle, sometimes energizing, sometimes reflective — match her mood but stay varied over time.`
  );
}
...
function buildChinVariationSystemNote(history = [], userText = "") {
  const usedVerses = extractUsedVerseReferences(history);
  const lines = [
    "THIS TURN — VARIATION (strict):",
    "Chin may repeat herself or ask something similar to an earlier message. Still give faith-based encouragement + at least one Bible verse.",
    "Use a DIFFERENT verse and a DIFFERENT approach than your previous replies in this chat (new angle, new practical tip, new tone).",
  ...
  if (/\b(?:bored|boring|burnout|what should i do)\b/.test(t) && history.length >= 2) {
    lines.push(
      "Topic hint: boredom / what to do — offer a fresh idea (short walk + prayer, switch subjects, worship break, review with a friend, etc.) and a verse you have NOT used yet."
    );
  }
  if (/\b(?:pass|passing|board|exam|makaka[\s-]?pass)\b/.test(t) && history.length >= 2) {
    lines.push(
      "Topic hint: passing the board exam — fresh angle only (perseverance, God's timing, doing your best as worship, peace vs striving, etc.) with a new verse."
    );
  }
  if (/\b(?:fear|afraid|anxious|anxiety|worried|kaba|panic)\b/.test(t) && history.length >= 2) {
    lines.push(
      "Topic hint: fear / anxiety — new comfort (God's presence, cast your cares, peace that guards hearts) with a verse not used before in this chat."
    );
  }
  return lines.join("\n");
}
...
async function generateChinEncouragementReply(userText, options = {}) {
  ...
  const variationNote = buildChinVariationSystemNote(safeHistory, userText);
  if (variationNote) {
