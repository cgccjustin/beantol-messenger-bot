 ...
 *
 * Roster (non-business messages → faith replies with mandatory Scripture block):
 *   Beantol:       Chin Siao
 *   Offbeat Brew:  Reyna Mae Epe, Honey Pearl Reyes, Justin Siao
 *
 * Business messages (orders, menu, beans, quotes, hours, etc.) use the normal bot.
 */
...
const PERSONAL_FAITH_TOPIC_PATTERN =
  /\b(?:feel(?:ing)?\s+(?:sad|down|low|bad|empty|lost|hopeless|lonely|tired|weak|broken|hurt|heavy|depressed|anxious|worried|scared|afraid|stressed|overwhelmed|burned?\s*out|discouraged|miserable|bleh|meh)|i(?:'m|\s+am)\s+(?:sad|depressed|lonely|anxious|worried|scared|afraid|stressed|tired|overwhelmed|hopeless|lost|broken|hurt|not\s+ok|not\s+okay|down|low)|depress(?:ed|ion|ing)?|mental\s+health|what\s+(?:should|can|do)\s+i\s+do\s+when|how\s+(?:should|can|do)\s+i\s+(?:cope|deal|handle|overcome)|need\s+(?:help|prayer|encouragement|comfort)|pray\s+for\s+me|prayer\s+request|bible|scripture|verse|god|jesus|christ|faith|church|forgive|forgiveness|salvation|spiritual|grief|grieving|mourning|heartbreak|break\s*up|relationship\s+problem|life\s+advice|purpose\s+in\s+life|why\s+(?:am\s+i|do\s+i)\s+(?:sad|here|suffering)|board\s+exam|reviewing\s+for|study\s+stress|exam\s+stress|burnout|bored\s+studying|scared\s+to\s+fail|afraid\s+(?:to\s+)?fail|kaba|kabado|malungkot|lungkot|nalulungkot|gikapoy|naluya|u\s?ban|gikaguol|gikulbaan|stress\s+kaayo|way\s+la(?:ng)?\s+paglaom)\b/i;
...
function isPersonalOrFaithTopic(text) {
  ...
  if (/\?\s*$/.test(t) && /\b(?:why|what|how|should|can)\b/i.test(t) && !OFFBEAT_OPERATIONAL_PATTERN.test(t) && !BEANTOL_OPERATIONAL_PATTERN.test(t)) {
    return /\b(?:feel|life|god|faith|pray|sad|depress|anxious|lonely|hope|peace|forgive|relationship|heart|mind|soul)\b/i.test(t);
  }
  return false;
}
...
function shouldUseFaithEncouragement(tenant, recipient, userText, options = {}) {
  if (!recipient || !isFaithEncouragementEnabled(tenant)) return false;
  return !isFaithProfileBusinessMessage(userText, tenant, recipient, options);
}
