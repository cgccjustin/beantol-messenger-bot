...
/** Personal / faith / life topics — always route to faith mode (never skip). */
const PERSONAL_FAITH_TOPIC_PATTERN =
  /\b(?:feel(?:ing)?\s+(?:sad|down|low|bad|empty|lost|hopeless|lonely|tired|weak|broken|hurt|heavy|depressed|anxious|worried|scared|afraid|stressed|overwhelmed|burned?\s*out|discouraged|miserable|bleh|meh)|i(?:'m|\s+am)\s+(?:sad|depressed|lonely|anxious|worried|scared|afraid|stressed|tired|overwhelmed|hopeless|lost|broken|hurt|not\s+ok|not\s+okay|down|low)|depress(?:ed|ion|ing)?|mental\s+health|what\s+(?:should|can|do)\s+i\s+do\s+when|how\s+(?:should|can|do)\s+i\s+(?:cope|deal|handle|overcome)|what\s+if\s+i|change\s+of\s+heart|have\s+a\s+change|new\s+heart|soft(?:en)?(?:ed)?\s+heart|hardened\s+heart|repent|repentance|redemption|salvation|doubting|struggling\s+with|lost\s+my\s+way|far\s+from\s+god|walk\s+with\s+god|need\s+(?:help|prayer|encouragement|comfort)|(?:can\s+you|please|pls)\s+(?:encourage|bless|inspire)|encourage\s+me|encouragement\s+(?:for|today|please)|pray\s+for\s+me|prayer\s+request|bible|scripture|verse|god|jesus|christ|faith|church|forgive|forgiveness|spiritual|grief|grieving|mourning|heartbreak|break\s*up|relationship\s+problem|life\s+advice|purpose\s+in\s+life|why\s+(?:am\s+i|do\s+i)\s+(?:sad|here|suffering)|board\s+exam|boards?\s+(?:exam|review|prep)|reviewing\s+for|study\s+stress|exam\s+stress|burnout|bored\s+studying|scared\s+to\s+fail|afraid\s+(?:to\s+)?fail|will\s+i\s+(?:pass|make\s+it|fail)|(?:pass|fail)\s+(?:the\s+)?(?:boards?|exam|PLE|NLE|BAR|CPA|LET|licensure)|PLE\b|NLE\b|don'?t\s+have\s+(?:much|enough)\s+time|running\s+out\s+of\s+time|not\s+(?:much|enough)\s+time\s+(?:left|to\s+study|to\s+review)|time\s+pressure|pressure\s+of|kaba|kabado|malungkot|lungkot|nalulungkot|gikapoy|naluya|u\s?ban|gikaguol|gikulbaan|stress\s+kaayo|way\s+la(?:ng)?\s+paglaom)\b/i;
...
function shouldUseFaithEncouragement(tenant, recipient, userText, options = {}) {
  ...
  const rosterRecipient = options.rosterRecipient || recipient;
  ...
  // Named roster member OR any customer on an open-to-all tenant:
  // faith mode for any message that is not clearly a business inquiry.
  const replyAs =
    rosterRecipient || buildGenericFaithRecipient(tenant, options.profileName);
  if (!replyAs) return false;

  if (rosterRecipient || isFaithEncouragementOpenToAll(tenant)) {
...
function shouldSkipFaithEncouragementForMessage(userText, tenant, recipient, options = {}) {
  ...
  // Explicit personal/faith topic — never skip.
  if (isPersonalOrFaithTopic(text)) return false;
  ...
  // Clear business / operational inquiry — route to the business bot.
  if (tenantId === "beantol") {
  ...
  // Anything else (greetings, questions, personal thoughts, vague messages) → faith.
  return false;
}
...
/**
 * Matches a greeting + vague help/question request on the same message.
 * "Hi can you help me?" / "Hello, I have a question" / "I need advice" etc.
 * Does NOT match specific questions (those fall to faith or business).
 */
const FIRST_MSG_AMBIGUOUS_PATTERN =
  /\b(?:can\s+(?:you\s+)?help(?:\s+me(?:\s+(?:out|with\s+something))?)?|help\s+me(?:\s+(?:with\s+)?(?:something|a\s+(?:problem|question|concern)))?|need\s+(?:some\s+)?(?:help|advice|guidance|someone\s+to\s+(?:talk\s+to?|listen))|have\s+(?:a\s+)?(?:question|concern|something\s+to\s+(?:ask|share))|want\s+to\s+(?:ask|talk)(?:\s+(?:about\s+)?(?:something|anything))?|may\s+(?:i\s+)?(?:ask\s+(?:something|you\s+something|a\s+question)?)|may\s+(?:tanong|itatanong|gusto|problema)\s*(?:ako|lang)?|meron\s+(?:akong\s+)?(?:tanong|gusto|problema)|pwede\s+(?:ba\s+)?(?:kita|kitang)\s+(?:tanungin|habulin)|need\s+to\s+talk|i\s+have\s+something\s+(?:to\s+(?:ask|say|share)))\b/i;
 ...
 *   "probe"    — send a gentle "are you here for business or personal?" nudge
 *   "faith"    — proceed with faith encouragement + brand intro
 */
function classifyFirstFaithMessage(text, tenant) {
  ...
  // Check for ambiguous BEFORE personal/faith so "hi can you help me?" probes
  // rather than jumping straight to faith
  if (FIRST_MSG_AMBIGUOUS_PATTERN.test(t)) return "probe";

  // Explicit personal/faith topic or clear non-business statement → faith with intro
  return "faith";
}

/**
 * Probe reply for ambiguous first messages — asks whether to help with business or personal.
 * Keeps the brand front and center per UX requirement.
 */
function buildFirstContactProbeReply(tenant, recipientName = "") {
  ...
  return (
    `${hi} I'm ${brand}'s chat assistant — happy to help.\n\n` +
    `Are you asking about ${productLabel}, or is there something personal I can help you with? ` +
    `Either way, I'm here. 😊`
  );
}
