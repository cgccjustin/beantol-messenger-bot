...
const FAITH_OPEN_TO_ALL_TENANTS = new Set(["beantol", "offbeat-brew", "kape-kristiano", "charity"]);
/** Tenants where every message is answered with faith encouragement (no business mode). */
const FAITH_ONLY_TENANTS = new Set(["charity"]);
...
function shouldUseFaithEncouragement(tenant, recipient, userText, options = {}) {
  ...
  // Faith-only tenants (e.g. Charity): every message is a faith reply — no topic filter.
