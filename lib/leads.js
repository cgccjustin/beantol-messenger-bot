...
function isLeadCaptureConfigured() {
  return isLeadCaptureEnabledForTenant() && hasGoogleCredentials();
}

function getSheetTab() {
  return getLeadsSheetTab();
}
...
async function prepareLeadsSheet(sheets, spreadsheetId, tab) {
...
async function recordLead(payload) {
  ...
  await prepareLeadsSheet(sheets, spreadsheetId, tab);
