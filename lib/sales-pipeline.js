...
function buildSalesContextNote(lead) {
  ...
  if (isStaleLead(lead)) {
    ...
    parts.push(
      "Active order intent — summarize items and guide the next step (quantity, pickup/delivery, payment). " +
        "If they digress with a FAQ, answer it then nudge back to completing the order."
    );
...
function renderSalesPipelineHtml({ active, stale, hot }, token = "", tenantId = "") {
...
<a class="btn btn-sm" href="/admin/sales/check-stale?token=${encodeURIComponent(token)}${tq}">Email stale list</a></p>
