...
function findOutOfStockProductsPromotedInReply(assistantReply) {
  ...
  return catalog.filter((p) => {
    ...
    const replyLower = reply.toLowerCase();
    const matchedToken = [p.label.toLowerCase(), ...(p.keys || [])].find((t) =>
      replyLower.includes(t)
    );

    // If the matched token is also a substring of an in-stock product label that IS in the reply,
    // the reply is talking about the in-stock variant (e.g. "Mt. Apo (Ellaga)" mentioned,
    // not "Mt. Apo (filter)"). Don't flag this as an OOS promotion.
    if (matchedToken) {
      const inStockOverlap = catalog.some((q) => {
        if (isProductIdOutOfStock(q.id)) return false;
        const qLabel = q.label.toLowerCase();
        return qLabel.includes(matchedToken) && replyLower.includes(qLabel);
      });
      if (inStockOverlap) return false;
    }
