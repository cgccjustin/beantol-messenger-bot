...
function getUnavailableProductIds() {
  if (isInventorySheetConfiguredLight()) {
    const { getCachedUnavailableProductIds } = require("./inventory-sheet");
    return getCachedUnavailableProductIds();
  }
...
function shouldBlockOutOfStockProductInquiry(userText) {
  ...
  if (isProductNameOnlyInquiry(userText)) return true;
  const t = String(userText || "").trim();
  if (t.length <= 72 && !/\b(?:and|or|,)\b/i.test(t)) return true;
  return false;
}
...
function assistantReplyTreatsOutOfStockProductAsBuyable(assistantReply, userText) {
  ...
  return (
    ...
    /\b(?:great|good|excellent|nice|perfect|wonderful)\s+(?:choice|pick|option|one)\b/i.test(
      reply
    ) ||
...
function buildInStockTasteRecommendationReply(userText) {
  ...
  lines.push("");
  lines.push("Which one would you like sizes and prices for?");
  return lines.join("\n");
}
