...
const APPOINTMENT_INTENT_PATTERNS = [
  /\b(?:appointment|cupping(?:\s+session)?|drop by|moadto sa shop|tan(-)?aw sa shop)\b/i,
  /\b(?:visit(?: the)? shop|shop visit)\b/i,
  /\b(?:book(?:ing)?|schedule).*(?:appointment|visit(?: the)? shop|shop visit|cupping|drop by)\b/i,
  /\b(?:appointment|visit(?: the)? shop|shop visit|cupping).*(?:book(?:ing)?|schedule)\b/i,
...
function isAppointmentIntent(text) {
  ...
  if (!t || isCustomerOwnLogisticsIntent(t)) return false;
  return APPOINTMENT_INTENT_PATTERNS.some((pattern) => pattern.test(t));
}
