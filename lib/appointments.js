...
const DATETIME_QUESTION =
  "Happy to help you book a visit at Beantol. Can you tell me your preferred date and time?";
...
function getBookingSession(senderId) {
  const s = bookingSessions.get(sessionKey(senderId));
  if (!s) return null;
...
async function processAppointmentFlow(senderId, userText, options = {}) {
  ...
  if (!session && isAppointmentIntent(text)) {
    ...
    return { handled: true, reply: DATETIME_QUESTION };
  }
  ...
  if (session.step === "datetime") {
    ...
    if (date && time) {
      session.date = date;
      session.time = time;
      return finalizeBooking(senderId, session, { ...options, name, phone, notes: text });
    }
    if (date) {
      session.date = date;
