...
const SIZE_PATTERN = /\b(250g|500g|1kg|6\s*kg|\d+(?:\.\d+)?\s*kg)\b/gi;
const BULK_KG_PATTERN = /\b(\d+(?:\.\d+)?)\s*kg\b/gi;

function parseKgAmount(text) {
  const match = String(text || "").match(/\b(\d+(?:\.\d+)?)\s*kg\b/i);
...
function bulkKgNearHit(source, hit) {
...
function sizeForProductHit(source, hit) {
  const bulkKg = bulkKgNearHit(source, hit);
...
function parseLineItemsFromText(text, options = {}) {
  ...
  for (const hit of hits) {
    ...
    let resolvedSize = sizeForProductHit(source, hit);
    let bulkKg = bulkKgNearHit(source, hit);

    if (wholesale || /\b(?:wholesale|bulk|6\s*kg)\b/i.test(window)) {
      if (hit.product.roast === "filter") continue;
      resolvedSize = "wholesale";
      bulkKg = bulkKg || WHOLESALE_MOQ_KG;
    }
...
function buildQuoteFromText(text, options = {}) {
  ...
  let lines = mergeQuoteLineItemsFromConversation(userSources, assistantReply, { wholesale });

  if (!lines.length) {
    lines = mergeLineItemsFromSources(userSources, { wholesale });
  }

  if (!lines.length && assistantReply) {
    lines = parseLineItemsFromText(assistantReply, { wholesale });
  }
  ...
  if (!lines.length) {
    ...
    const sizeMatch = combined.match(/\b(250g|500g|1kg|6\s*kg|\d+(?:\.\d+)?\s*kg)\b/i);
    let resolvedSize = normalizeSize(size) || (sizeMatch ? normalizeSize(sizeMatch[0]) : "");
    let bulkKg = parseKgAmount(combined);
