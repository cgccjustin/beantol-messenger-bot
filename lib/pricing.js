...
function buildRetailBulkLineItem(product, kg) {
  if (!Number.isFinite(kg) || kg <= 0 || kg >= WHOLESALE_MOQ_KG) return null;
  const prices = RETAIL_PRICES[product.id];
...
function requestedBelowMoqBulkKg(text, recentTexts = []) {
...
function parseLineItemsFromText(text, options = {}) {
  ...
  for (const hit of hits) {
    ...
    if (
      ...
      !requestedBelowMoqBulkKg(quoteUserText || source)
    ) {
...
function mergeQuoteLineItemsFromConversation(userSources, assistantReply, options = {}) {
  ...
  if (assistantReply) {
    const blockAssistantWholesale = requestedBelowMoqBulkKg(options.quoteUserText || "");
...
function buildQuoteFromText(text, options = {}) {
  ...
  if (!lines.length) {
    ...
    const belowMoqKg = requestedBelowMoqBulkKg(combined);
    if (belowMoqKg) {
    ...
    if (
      ...
      (/\b(?:wholesale|bulk)\b/i.test(combined) &&
        !requestedBelowMoqBulkKg(combined))
    ) {
