...
function buildWholesaleOrderPricingSystemNote(userText, recentTexts = [], sessionQuote = null) {
  const effective = resolveEffectiveOrderKg(userText, recentTexts, sessionQuote);
  if (!effective || effective.kg < WHOLESALE_MOQ_KG) return "";
  ...
  const upgradeNote = buildWholesalePricingSystemNote(userText, recentTexts, sessionQuote);
  if (upgradeNote) return upgradeNote;

  const combined = [userText, ...(recentTexts || [])].filter(Boolean).join("\n");
  const orderIntent =
    /\b(?:deliver|delivery|padala|ship|order|buy|purchase|checkout|proceed|go ahead|today|karon|now)\b/i.test(
      combined
    ) || customerWholesaleIntent(userText, sessionQuote);

  if (!orderIntent && !matchCatalogFromText(userText)) return "";

  return (
    `WHOLESALE PRICING (strict — ${effective.kg} kg order): Customer is ordering ${effective.kg} kg of ${product.label} (6 kg+ wholesale MOQ). ` +
    `Quote ONLY wholesale ${formatPeso(wholesalePrice)}/kg × ${effective.kg} kg = ${formatPeso(wholesalePrice * effective.kg)} total. ` +
    `Do NOT quote retail pack prices (250g/500g/1kg) for this bulk order. ` +
    (retail1kg
      ? `Retail 1 kg is ${formatPeso(retail1kg)} — do NOT use that rate for ${effective.kg} kg wholesale.`
      : "")
  );
}

function resolveProductForPricingNote(userText, recentTexts = [], sessionQuote = null) {
  const fromSession = productById(sessionQuote?.lineItems?.[0]?.productId);
  if (fromSession) return fromSession;

  const { isProductIdOutOfStock } = require("./inventory-availability");
  const pickProduct = (text, { allowOutOfStock = false } = {}) => {
    const product = matchCatalogFromText(String(text || ""));
    if (!product) return null;
    if (!allowOutOfStock && isProductIdOutOfStock(product.id)) return null;
    return product;
  };

  const fromCurrent = pickProduct(userText, { allowOutOfStock: true });
  if (fromCurrent) return fromCurrent;

  const recents = Array.isArray(recentTexts) ? recentTexts : [];
  for (let i = recents.length - 1; i >= 0; i--) {
    const hit = pickProduct(recents[i]);
    if (hit) return hit;
  }

  return matchCatalogFromText([userText, ...recents].filter(Boolean).join(" "));
}
...
/** Wholesale tier only when the customer (not the AI) asked for bulk/wholesale/kg. */
function customerWholesaleIntent(quoteUserText, sessionQuote = null) {
  const t = String(quoteUserText || "").trim();
  if (!t) return false;
  if (requestedBelowMoqBulkKg(t, [], sessionQuote)) return false;
  if (customerMentionedWholesale(t)) return true;
  const kg = resolveOrderKg(t);
  return kg != null && kg >= WHOLESALE_MOQ_KG;
}

function customerHasExplicitSizeInQuoteContext(parseOpts) {
  const t = String(parseOpts.quoteUserText || "").trim();
  if (!t) return false;
  if (parseOpts.effectiveKg) return true;
...
function sizeForProductHit(source, hit, options = {}) {
  ...
  return bestSize ? clampSizeForProduct(hit.product, bestSize) : "";
}
...
function parseLineItemsFromText(text, options = {}) {
  ...
  for (const hit of hits) {
    ...
    if (
      !bulkKg &&
      customerWholesaleIntent(quoteUserText || "", sessionQuote) &&
    ...
    if (!resolvedSize && !bulkKg) continue;

    if (!bulkKg && !customerHasExplicitSizeInQuoteContext(parseOpts)) {
      const explicit = explicitSizeForProduct(source, hit);
...
const QUOTE_SUMMARY_BLOCK_PATTERN =
  /Quote summary\s*—\s*please confirm:\s*\n+([\s\S]*?)\n\nTotal:\s*[^\n]+/i;

function extractQuoteSummaryBullets(assistantReply) {
  const match = String(assistantReply || "").match(QUOTE_SUMMARY_BLOCK_PATTERN);
...
/** Parse only the structured "Quote summary — please confirm" block (not full chat). */
function buildQuoteFromConfirmSummary(assistantReply, options = {}) {
  const bullets = extractQuoteSummaryBullets(assistantReply);
  if (!bullets?.length) return null;

  const byProduct = new Map();
  for (const bullet of bullets) {
    const parseText = bullet.split(/\s*=\s*₱/)[0].trim();
    if (!parseText) continue;
    for (const line of parseLineItemsFromText(parseText, options)) {
      byProduct.set(line.productId, line);
    }
  }

  const lines = Array.from(byProduct.values());
...
function mergeQuoteLineItemsFromConversation(userSources, assistantReply, options = {}) {
  ...
  if (assistantReply) {
    ...
    for (const line of parseLineItemsFromText(assistantReply, options)) {
      ...
      if (
        line.size === "wholesale" &&
        !customerWholesaleIntent(options.quoteUserText || "", options.sessionQuote)
      ) {
        continue;
      }
