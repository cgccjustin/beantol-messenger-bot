...
function isoDateInShopTz(date = new Date()) {
  return new Intl.DateTimeFormat("en-CA", {
    timeZone: SHOP_TIMEZONE,
    year: "numeric",
    month: "2-digit",
...
function closureOnDate(closures, isoDate) {
  return (closures || []).find((c) => c.date === isoDate) || null;
}

function dayIndexForIsoDate(isoDate) {
  const [y, m, d] = isoDate.split("-").map(Number);
  const { getShopLocalParts } = require("./shop-hours");
  return getShopLocalParts(new Date(Date.UTC(y, m - 1, d, 12, 0, 0))).dayIndex;
}

function isRegularShopDay(tenant, isoDate) {
  const { getTenantSchedule } = require("./shop-hours");
  return getTenantSchedule(tenant).openDays.includes(dayIndexForIsoDate(isoDate));
}

/**
 * Physical shop open on a calendar day (ignores time-of-day except when isoDate is today).
 * Returns { open, closure, regularClosed, schedule }.
 */
function evaluateShopDay(tenant, isoDate, closures = [], now = new Date()) {
  const { getTenantSchedule, isShopOpenNow } = require("./shop-hours");
  const schedule = getTenantSchedule(tenant);
  const closure = closureOnDate(closures, isoDate);
  const regularClosed = !isRegularShopDay(tenant, isoDate);
  const todayIso = isoDateInShopTz(now);

  if (closure) {
    return { open: false, closure, regularClosed, schedule, isoDate };
  }
  if (regularClosed) {
    return { open: false, closure: null, regularClosed: true, schedule, isoDate };
  }
  if (isoDate === todayIso) {
    return {
      open: isShopOpenNow(tenant, now),
      closure: null,
      regularClosed: false,
      schedule,
      isoDate,
  ...
  return { open: true, closure: null, regularClosed: false, schedule, isoDate };
}

function nextOpenShopDayHint(tenant, fromDate = new Date(), closures = []) {
  const { getTenantSchedule, getShopLocalParts, formatCloseHour } = require("./shop-hours");
  const schedule = getTenantSchedule(tenant);
  const openLabel = formatCloseHour(schedule.openHour);
  const todayIso = isoDateInShopTz(fromDate);
  const { dayIndex: todayIdx, hour, minute } = getShopLocalParts(fromDate);
  const mins = hour * 60 + minute;
  const beforeOpenToday =
    schedule.openDays.includes(todayIdx) &&
    mins < schedule.openHour * 60 &&
    !closureOnDate(closures, todayIso);

  for (let offset = beforeOpenToday ? 0 : 1; offset <= 14; offset++) {
    const iso = addDays(todayIso, offset);
    if (closureOnDate(closures, iso)) continue;
    if (!isRegularShopDay(tenant, iso)) continue;
    const [y, m, d] = iso.split("-").map(Number);
    const weekday = getShopLocalParts(new Date(Date.UTC(y, m - 1, d, 12, 0, 0))).weekday;
    if (offset === 0) return `today at ${openLabel}`;
    if (offset === 1) return `tomorrow at ${openLabel}`;
    return `${weekday} at ${openLabel} (${formatClosureDate(iso)})`;
  }
  return openLabel;
}
