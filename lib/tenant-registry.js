...
function normalizeFaithSenderId(senderId) {
  ...
  if (typeof senderId === "number" && !Number.isSafeInteger(senderId)) {
    console.warn(
      `Faith sender ID ${senderId} exceeds JS safe integer — use a quoted string in tenant config (e.g. "${String(senderId)}").`
    );
  }
  return String(senderId).trim();
}

function normalizeFaithEncouragementFeature(raw) {
  if (raw === false) return { enabled: false, recipients: [] };
        ...
        .map((entry) => {
          ...
          return {
            ...
            names: Array.isArray(entry?.names)
              ...
              : [],
            senderIds: (entry?.senderIds || [])
              .map(normalizeFaithSenderId)
              .filter(Boolean),
          };
        })
        .filter(Boolean)
...
function normalizeTenant(raw) {
  ...
  const faithEncouragement = normalizeFaithEncouragementFeature(raw.features?.faithEncouragement);
  ...
  return {
    ...
    features: {
      ...
      ...(faithEncouragement ? { faithEncouragement } : {}),
      ...(raw.features?.chinEncouragement === false ? { chinEncouragement: false } : {}),
    },
    rules: {
...
function loadTenantRegistry() {
  ...
  for (const t of registry.tenants) {
    const faith = t.features?.faithEncouragement;
    const faithNote =
      faith?.recipients?.length
        ? `, faith: ${faith.recipients.length} profile(s), ${faith.recipients.filter((r) => r.senderIds?.length).length} with sender IDs${faith.openToAll ? ", openToAll: ON" : ""}`
        : faith?.openToAll
          ? ", faith: openToAll: ON"
          : "";
    console.log(`  - ${t.id}: ${t.name}${faithNote}`);
  }
  const envOpenToAll = String(process.env.FAITH_ENCOURAGEMENT_OPEN_TO_ALL || "")
    .trim()
    .toLowerCase();
  if (envOpenToAll === "true") {
    console.log(
      "Faith encouragement: OPEN TO ALL customers on Beantol, Offbeat Brew, and Kape Kristiano (FAITH_ENCOURAGEMENT_OPEN_TO_ALL=true)"
    );
  } else if (envOpenToAll === "false") {
    console.log("Faith encouragement: roster profiles only (FAITH_ENCOURAGEMENT_OPEN_TO_ALL=false)");
  }
  return registry;
}
