...
/** OpenAI rejects null/non-string content — filter bad rows from sheet or cache. */
function sanitizeMessagesForOpenAi(messages) {
  const validRoles = new Set(["user", "assistant"]);
  return (messages || []).filter((m) => {
    if (!m || !validRoles.has(m.role)) return false;
 ...
 */
async function prewarmHistory(senderId) {
  const key = scopeKey(senderId);
  if (!warmedKeys.has(key)) {
    await warmFromSheet(key);
  }
}
