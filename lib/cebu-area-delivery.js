...
/** Customer will book their own rider/courier — not a shop-visit appointment. */
const CUSTOMER_OWN_LOGISTICS_PATTERNS = [
  /\b(?:i(?:'ll|\s+will)|ako\s+na\s+mag|mag-?book|book(?:ing)?|arrange|ipa(?:\s|-)?(?:pick\s*up|kuha|deliver)|pabuhatan|send).{0,50}\b(?:delivery(?:\s+person|\s+rider)?|rider|courier|logistics|grab|maxim|lalamove|padala|hatod)\b/i,
  /\b(?:delivery(?:\s+person|\s+rider)?|rider|courier|logistics).{0,50}\b(?:book(?:ing)?|arrange|ipa(?:\s|-)?(?:pick\s*up|kuha|deliver)|pabuhatan)\b/i,
  /\b(?:my own|own)\s+(?:delivery|logistics|rider|courier)\b/i,
  /\bi(?:'ll|\s+will)\s+arrange\s+(?:my own\s+)?(?:delivery|logistics|rider|courier)\b/i,
];

function isCustomerOwnLogisticsIntent(text) {
  const t = String(text || "").trim();
  return CUSTOMER_OWN_LOGISTICS_PATTERNS.some((pattern) => pattern.test(t));
}
...
function getCebuDeliverySystemNote() {
  return (
    ...
    "- NEVER mention Lalamove, GrabFood, Foodpanda, or other courier apps unless the customer named them first — local Cebu delivery is Maxim only.\n" +
