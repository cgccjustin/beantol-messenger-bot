...
// in any language. Used to penalise non-availability FAQs (e.g. cupping sessions) that
// share the token "beans" with the query but are answering a completely different question.
const AVAILABILITY_QUERY =
  /\b(?:naa|available|availability|in\s+stock|stock|meron|mayroon|have|unsay\s+naa|what(?:'s|\s+is|\s+are)?\s+(?:there|available|in\s+stock)|what\s+(?:beans?|coffee)?\s*(?:do\s+you|have|have\s+you))\b/i;
...
function adjustFaqScore(userText, faq, baseScore) {
  ...
  // Heavy penalty: cupping-session FAQs should never match availability queries.
  // A customer asking "what beans do you have?" / "Unsay Naa nga beans?" is asking
  // about stock, not about café cupping sessions. Without this penalty the single
  // shared token "beans" in "Sample beans?" scores exactly MIN_MATCH_SCORE and the
  // cupping answer is returned instead of passing through to the AI with inventory context.
  if (CUPPING_FAQ_HINT.test(qNorm) && !CUPPING_FAQ_HINT.test(uNorm)) {
    score -= 10;
  }
  // Symmetric: availability-type queries should not match cupping FAQs.
  if (AVAILABILITY_QUERY.test(uNorm) && CUPPING_FAQ_HINT.test(qNorm)) {
    score -= 10;
  }
