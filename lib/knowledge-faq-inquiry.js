...
/**
 * Returns the best FAQ match along with its confidence score.
 * Callers can use this to enforce a minimum confidence threshold before
 * returning the answer — preventing weak single-token matches from
 * overriding the AI for ambiguous queries.
 */
function findBestFaqMatchWithScore(tenant, userText) {
  const faqs = loadAllFaqs(tenant);
  if (!faqs.length) return null;

  const multiWho = tryMultiWhoIsReply(faqs, userText);
  if (multiWho) return { answer: multiWho, score: 999 }; // WHO_IS always high confidence

  const match = findBestFaqMatch(faqs, userText);
  if (!match) return null;

  return { answer: match.faq.answer, score: match.score };
}
