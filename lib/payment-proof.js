...
function markPendingPaymentProof(senderId, text) {
  pendingPaymentProof.set(scopeKey(senderId), { at: Date.now(), text: String(text || "") });
}

function clearPendingPaymentProof(senderId) {
  pendingPaymentProof.delete(scopeKey(senderId));
}

function hasPendingPaymentProof(senderId) {
  const entry = pendingPaymentProof.get(scopeKey(senderId));
...
function markPaymentProofHandled(senderId) {
  paymentProofHandledAt.set(scopeKey(senderId), Date.now());
  clearPendingPaymentProof(senderId);
}

const QUOTE_CONFIRM_YES_PATTERN =
  /^(?:yes|yep|yeah|yup|oo|opo|ok(?:ay)?|confirm(?:ed)?|proceed|go ahead|sige|👍|✅)$/i;

function shouldSuppressAfterPaymentProof(senderId, userText = "") {
  if (QUOTE_CONFIRM_YES_PATTERN.test(String(userText || "").trim())) return false;

  const at = paymentProofHandledAt.get(scopeKey(senderId));
  if (!at) return false;
  if (Date.now() - at > PAYMENT_HANDLED_SUPPRESS_MS) {
    paymentProofHandledAt.delete(scopeKey(senderId));
    return false;
  }
  return true;
}
...
function resolvePaymentProofSubmission(userText, options = {}) {
  ...
  if (senderId && shouldSuppressAfterPaymentProof(senderId, userText)) {
  ...
  if (!hasImageAttachment && (explicitHere || pending)) {
    const entry = senderId ? pendingPaymentProof.get(scopeKey(senderId)) : null;
    const waitedLong =
