...
function markOutsideCebuAgentOffered(senderId) {
  ...
  session.at = Date.now();
  sessions.set(scopeKey(senderId), session);
  agentOfferPending.set(scopeKey(senderId), Date.now());
}

function isOutsideCebuAgentOfferPending(senderId) {
  const at = agentOfferPending.get(scopeKey(senderId));
  if (!at) return false;
  if (Date.now() - at > AGENT_OFFER_TTL_MS) {
    agentOfferPending.delete(scopeKey(senderId));
    return false;
  }
  return true;
}
...
function getOutsideCebuSystemNote() {
  return (
    ...
    "- NEVER mention Lalamove, GrabFood, Foodpanda, or other courier apps unless the customer named them first."
  );
}
