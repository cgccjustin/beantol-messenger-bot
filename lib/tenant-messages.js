...
const DEFAULT_HANDOFF_WHILE_WAITING =
...
function appendHandoffWhileWaitingNote(reply) {
  const text = String(reply || "").trim();
  if (!text) return DEFAULT_HANDOFF_WHILE_WAITING;
  if (/while you wait|still here to help|keep asking questions/i.test(text)) return text;
  return `${text}\n\n${DEFAULT_HANDOFF_WHILE_WAITING}`;
}
...
function getHandoffReply(tenant) {
  ...
  if (custom) return appendHandoffWhileWaitingNote(custom);
  if (process.env.HANDOFF_REPLY?.trim()) {
    return appendHandoffWhileWaitingNote(process.env.HANDOFF_REPLY.trim());
  }
