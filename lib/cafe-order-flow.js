...
function tryResumeCafeOrderFlow(senderId, userText, tenant, options = {}) {
  ...
  if (isOffTopicCafeOrderMessage(userText, tenant)) return { resumed: false };
  ...
  if (askedFulfillment) {
    ...
    if (!resolvedFulfillment && hints?.fulfillment) resolvedFulfillment = hints.fulfillment;
    if (!resolvedFulfillment) resolvedFulfillment = detectCafeFulfillment(historyText);
...
function processCafeOrderFlowPreAi(senderId, userText, tenant, options = {}) {
  ...
  if (isOffTopicCafeOrderMessage(text, tenant)) {
