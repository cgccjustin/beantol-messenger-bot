...
function looksLikeCafeDeliveryDetails(text) {
  ...
  if (lines.length >= 2 && lines.some((line) => isPhoneOnlyLine(line))) return true;
...
function findLastPhoneOnlyLineIndex(lines) {
  for (let i = lines.length - 1; i >= 0; i--) {
    if (isPhoneOnlyLine(lines[i])) return i;
  }
  return -1;
}
...
function isDeliveryFragmentMessage(text) {
  ...
  if (/^(?:pickup|delivery|deliver|padala|hatod)$/i.test(t)) return false;
  if (isPhoneOnlyLine(t)) return true;
  if (/\b(?:offbeat|kape kristiano|order ko|payment|gcash|unsay|mode of)\b/i.test(t)) return false;
  if (looksLikeCafeDeliveryDetails(t) && t.length <= 120) return true;
...
function extractLatestDeliveryBlock(recentUserTexts = [], userText = "") {
  ...
  for (let count = Math.min(5, messages.length); count >= 1; count--) {
    const blob = messages.slice(-count).join("\n");
    if (looksLikeCafeDeliveryDetails(blob)) return blob;
  }
  const full = messages.join("\n");
  if (looksLikeCafeDeliveryDetails(full)) return full;
  return String(userText || "").trim();
}

function parseMultilineDeliveryDetails(lines, phoneFromText) {
  const phoneLineIdx = findLastPhoneOnlyLineIndex(lines);
...
function parseInlineDeliveryBlob(text, phone) {
  ...
  if (phone) {
    working = working.replace(phone, " ").replace(/\s+/g, " ").trim();
  }
...
function hasCafeOrderIntent(text, historyTexts = [], tenant, options = {}) {
  ...
  const combined = combineOrderContext(text, historyTexts);
  if (CAFE_ORDER_INTENT.test(combined)) return true;
  if (!parseCafeOrderFromText(text, tenant)?.length) return false;
...
function tryStartCafeOrderFlow(senderId, userText, tenant, options = {}) {
  ...
  let fulfillment = detectCafeFulfillment(contextText);
  if (!fulfillment && hints?.fulfillment) fulfillment = hints.fulfillment;
  const paymentAsk = asksPaymentMode(contextText) || Boolean(hints?.paymentAsk);
...
function tryResumeCafeOrderFlow(senderId, userText, tenant, options = {}) {
  ...
  if (askedFulfillment) {
    const hints = peekCafeOrderHints(senderId);
    let fulfillment = detectCafeFulfillment(historyText);
    if (!fulfillment && hints?.fulfillment) fulfillment = hints.fulfillment;
    const deliveryBlock = extractLatestDeliveryBlock(recentUserTexts, userText);
    const step =
      fulfillment === "delivery"
...
function processCafeOrderFlowPreAi(senderId, userText, tenant, options = {}) {
  ...
  if (session.step === "delivery_collect") {
    const detailsSource = extractLatestDeliveryBlock(recentUserTexts, text);
