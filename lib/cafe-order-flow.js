...
function buildOrderConfirmationReply(session, tenant, options = {}) {
  ...
  if (fulfillment === "delivery" && details) {
    lines.push(
      "",
      "Delivery:",
      `• Name: ${details.name || "(please confirm)"}`,
      `• Address: ${details.address || details.raw || "(please confirm)"}`,
      `• Contact: ${details.phone || "(please confirm)"}`
    );
...
function buildMixedProductQuestionNote(userText, tenant) {
  ...
  return (
    ...
    " Then invite them to continue (quantity, pickup or delivery). Do NOT skip the explanation or jump straight to quantity."
  );
}
...
function getProductBlurb(tenant, productId) {
  return PRODUCT_BLURBS[tenant?.id]?.[productId] || "";
}
...
function buildMixedProductOrderReply(items, tenant) {
  ...
  for (const item of items) {
    ...
    lines.push(blurb || `${item.label} — one of our menu drinks.`);
  }
  lines.push("", "How many would you like? Reply with a number (e.g. 1 or 2 bottles).");
  return lines.join("\n\n");
}
...
function lastReplyDescribesProduct(lastAssistantReply) {
  const last = String(lastAssistantReply || "");
  return /\b(?:cold brew|creamy|velvety|house milk|signature cold brew|offbeat white|offbeat black|our menu features|ready-to-drink bottled)\b/i.test(
    last
  );
}
...
function buildCafeMenuReply(tenant) {
  if (tenant?.id === "offbeat-brew") {
    ...
    return [
      ...
      "",
      "*Coffee Series (Cold Brew Only)*:",
      ...coffee.map((p) => formatMenuLine(p, tenant)),
      "",
      "*Unplugged Series* (non-coffee):",
      ...unplugged.map((p) => formatMenuLine(p, tenant)),
      "",
...
function messageAnswersCurrentOrderStep(text, session, tenant) {
  ...
  if (session.step === "confirm_quantity") {
    return Boolean(parseQuantityFromReply(t, session.items) || isOrderAcknowledgment(t));
  }
...
function shouldAppendCafeOrderNudge(reply, session) {
  ...
  if (
    /\b(?:would you still like|would you like to proceed|how many bottles|pickup or delivery|reply pickup|send delivery details|gcash|payment proof|on file)\b/.test(
      r
    )
  ) {
    return false;
...
function tryStartCafeOrderFlow(senderId, userText, tenant, options = {}) {
  ...
  if (hasMixedProductQuestion(userText)) {
    ...
    if (items.length && orderish) {
      ...
      return {
        started: true,
        reply: buildMixedProductOrderReply(items, tenant),
        captureOrder: {
          ...captureBase,
          orderStatus: "inquiry",
  ...
  const deliveryBlock = extractLatestDeliveryBlock(recentUserTexts, userText);
  const captureBase = {
    cafeOrderCapture: true,
    isOrderIntent: true,
    bean: items.map((i) => `${i.qty}× ${i.label}`).join(", "),
...
function tryAdvanceSessionFromRichMessage(senderId, session, text, tenant, options = {}) {
  ...
  const block = extractLatestDeliveryBlock(recentUserTexts, text);
...
function processCafeOrderFlowPreAi(senderId, userText, tenant, options = {}) {
  ...
  if (session.step === "choose_fulfillment") {
    ...
    if (choice === "delivery") {
      session.fulfillment = "delivery";
      session.updatedAt = Date.now();
      const deliveryBlock = extractLatestDeliveryBlock(recentUserTexts, text);
  ...
  if (session.step === "delivery_collect") {
    let detailsSource = extractLatestDeliveryBlock(recentUserTexts, text);
    if (!looksLikeCafeDeliveryDetails(detailsSource)) {
      detailsSource = extractLatestDeliveryBlock(recentUserTexts.slice(-6), text);
    }
    if (looksLikeCafeDeliveryDetails(detailsSource)) {
