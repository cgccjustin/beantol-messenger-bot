...
const FULFILLMENT_KEYWORD_LINE = /^(?:pickup|delivery|deliver|padala|hatod|maxim|grab|lalamove|jnt|j&t|lbc|flash\s*express|angkas|go\s*padala)$/i;

/** Known delivery-service brand names that should never be mistaken for a contact name. */
const DELIVERY_SERVICE_NAME = /^(?:maxim|grab|lalamove|jnt|j&t|lbc|flash\s*express|angkas|go\s*padala)$/i;
...
function looksLikePersonNameLine(line) {
  ...
  if (isOrderAcknowledgment(t) || isShortAffirmation(t)) return false;
  // Delivery service brand names are not contact names.
  if (DELIVERY_SERVICE_NAME.test(t)) return false;
 ...
 * has a stalled (non-active) cafe order session. Returns "" if nothing is pending.
 * This is intentionally soft — faith replies are never interrupted mid-flow.
 */
/**
 * Returns a nudge for an ACTIVE (recently-touched) cafe session.
 * Used when faith fires despite an active order because the message was purely personal.
 */
function buildActiveOrderFaithNudge(senderId, tenant) {
  if (!isCafeOrderFlowEnabled(tenant)) return "";
  const session = getSession(senderId);
  if (!session || session.step === "closed" || !session.items?.length) return "";
  if (sessionWasIdle(session)) return ""; // idle → use buildFaithPendingNudge instead
  const summary = session.items.map((i) => `${i.qty}× ${i.label}`).join(", ");
  const stepPrompts = {
    confirm_quantity: `how many would you like`,
    choose_fulfillment: `pickup or delivery?`,
    delivery_collect: `your delivery details (address, name, mobile)`,
    confirm_order: `just reply OK to confirm`,
    awaiting_payment: `send payment proof when ready`,
  };
  const hint = stepPrompts[session.step] || `we can continue your order`;
  return `\n\nP.S. Coming back to your order — I still have ${summary} here. Whenever you're ready: ${hint}! 😊`;
}

/**
 * Returns a gentle postscript nudge to append to a faith reply when the customer
 * has a stalled (idle) cafe order session. Returns "" if nothing is pending or
 * if the session is still recently active.
 */
function buildFaithPendingNudge(senderId, tenant) {
  if (!isCafeOrderFlowEnabled(tenant)) return "";
  ...
  const summary = session.items.map((i) => `${i.qty}× ${i.label}`).join(", ");
  return `\n\nP.S. Whenever you're ready — no rush at all — I still have your order here (${summary}). Just say the word! 😊`;
}

/**
 * Returns true when a cafe order session exists AND is recently active (not idle).
 * Use this to decide whether faith mode should be blocked — idle sessions should
 * let faith through (with a nudge), while mid-flow sessions should not.
 */
function isCafeOrderFlowRecentlyActive(senderId) {
  const session = getSession(senderId);
  if (!session || session.step === "closed") return false;
  return !sessionWasIdle(session);
}
