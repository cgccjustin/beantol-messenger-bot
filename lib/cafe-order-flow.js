...
function extractPhoneFromMessageHistory(recentUserTexts = [], userText = "") {
  for (const msg of [...recentUserTexts, userText].reverse()) {
    ...
    if (!t) continue;
    if (isPhoneOnlyLine(t) || (lineHasPhone(t) && t.replace(/\s+/g, "").length <= 16)) {
      const phone = extractPhone(t);
...
function resolveDeliveryDetails(existing, recentUserTexts = [], userText = "", tenant = null) {
  ...
  const parsed =
    (block && looksLikeCafeDeliveryDetails(block) ? parseDeliveryDetails(block) : null) ||
    parsePartialDeliveryBlock(recentUserTexts, normalizedText, tenant);
  ...
  return finalizeDeliveryDetails(
    {
      ...merged,
      phone,
      raw: merged.raw || block || combined,
    },
    merged.raw || block || combined
  );
}
...
const CAFE_ORDER_CHANGE_PATTERN =
  /\b(?:something else|different|change(?:\s+my)?\s+order|not that|don't want|dont want|do not want|other drink|another drink|instead|no i want|no,?\s*i want|wag na|dili na|lahi|iba(?:ng)?)\b/i;
...
function buildGenericOrderStartReply(tenant) {
  const menu = buildCafeMenuReply(tenant);
  if (menu) {
    return `${menu}\n\nWhat would you like to order? Reply with the drink name.`;
  }
  return "What would you like to order? Tell me the drink name and how many you'd like.";
}
...
function buildQuantityReprompt(items, tenant) {
  const labels = items.map((i) => i.label).join(", ");
  return [
    `Reply with how many ${labels} you'd like${quantityUnitHint(tenant)}, or tell me a different drink from our menu.`,
  ].join("\n");
}
...
function processCafeOrderFlowPreAi(senderId, userText, tenant, options = {}) {
  ...
  if (
    ...
    isGenericCafeOrderInquiry(text, tenant) &&
  ...
  if (session.step === "confirm_quantity") {
    ...
    if (isCafeOrderChangeRequest(text) || (isGenericCafeOrderInquiry(text, tenant) && !qty)) {
      resetCafeOrderContext(senderId);
      return {
        handled: true,
        reply: buildGenericOrderStartReply(tenant),
  ...
  if (session.step === "delivery_collect") {
    ...
    if (details && (details.address || details.name)) {
      ...
      return {
        handled: true,
        reply: buildPhoneCollectPrompt(tenant, details),
      };
    }
