...
function parsePartialDeliveryBlock(recentUserTexts = [], userText = "", tenant = null) {
  ...
  if (!t || extractPhone(t)) return null;
  const lines = t.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
...
function processCafeOrderFlowPreAi(senderId, userText, tenant, options = {}) {
  ...
  if (session.step === "confirm_order") {
    if (isOrderAcknowledgment(text) || /^confirm(ed)?$/i.test(text)) {
      const details = finalizeDeliveryDetails(
        session.deliveryDetails || {},
        session.deliveryDetails?.raw || ""
      );
    ...
    if (lineHasPhone(text) && (isPhoneOnlyLine(text) || text.length <= 24)) {
      ...
      if (phone) {
        session.deliveryDetails = finalizeDeliveryDetails(
          { ...(session.deliveryDetails || {}), phone, raw: session.deliveryDetails?.raw || text },
          session.deliveryDetails?.raw || text
        );
        touchSessionActivity(session);
        sessions.set(scopeKey(senderId), session);
        return {
    ...
    if (looksLikeCafeDeliveryDetails(block)) {
      session.deliveryDetails = mergeDeliveryDetails(
        session.deliveryDetails,
        parseDeliveryDetails(block)
      );
    }
    touchSessionActivity(session);
    sessions.set(scopeKey(senderId), session);
  ...
  if (session.step === "delivery_collect") {
    ...
    if (looksLikeCafeDeliveryDetails(detailsSource)) {
      const details = mergeDeliveryDetails(
        session.deliveryDetails,
        parseDeliveryDetails(detailsSource)
      );
      session.step = "confirm_order";
      session.fulfillment = "delivery";
      session.deliveryDetails = details;
    ...
    const partial =
      parsePartialDeliveryBlock(recentUserTexts, text, tenant) ||
      (looksLikePartialCafeDeliveryDetails(text) ? parsePartialDeliveryBlock([], text, tenant) : null);
    if (partial) {
      session.deliveryDetails = mergeDeliveryDetails(session.deliveryDetails, partial);
      session.fulfillment = "delivery";
      touchSessionActivity(session);
      sessions.set(scopeKey(senderId), session);
    }

    if (lineHasPhone(text) && session.deliveryDetails) {
      const details = finalizeDeliveryDetails(
        { ...session.deliveryDetails, phone: extractPhone(text), raw: session.deliveryDetails.raw || text },
        session.deliveryDetails.raw || text
      );
      if (details.phone && (details.address || details.name)) {
        session.step = "confirm_order";
        session.deliveryDetails = details;
        touchSessionActivity(session);
        sessions.set(scopeKey(senderId), session);
        return {
          ...
          captureOrder: {
            ...
            bean: session.items.map((i) => `${i.qty}× ${i.label}`).join(", "),
            lineItems: formatLineItemsSummary(session.items, tenant),
            trigger: "cafe order delivery phone",
          },
        };
      }
    }

    if (session.deliveryDetails?.address || session.deliveryDetails?.name) {
      return {
        handled: true,
        reply: buildPhoneCollectPrompt(tenant, session.deliveryDetails),
      };
    }
